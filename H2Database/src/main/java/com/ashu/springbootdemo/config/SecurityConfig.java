...
//		http.csrf(csrf -> csrf.disable());
		...
		http.csrf((csrf) -> csrf.ignoringRequestMatchers(AntPathRequestMatcher.antMatcher("/saveMsg"))
				...
				.authorizeHttpRequests(
						authorize -> authorize.requestMatchers(AntPathRequestMatcher.antMatcher("/logout")).permitAll())
	...
	public InMemoryUserDetailsManager userDetailsService() {
		...
		UserDetails admin = User.withDefaultPasswordEncoder().username("admin").password("Demo@1234").roles("ADMIN")
				.build();
